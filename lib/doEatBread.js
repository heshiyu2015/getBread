...
module.exports = function(url,cb){
  http.get(url,function(res){
    ...
    res.on('end',function(){
      ...
      var whichday_for_head = [];//记录那一天用于生成导航栏
	...
	var oneDay = [];//装载这一天的内容
        whichday_for_head.push((($(day).attr('id')).split('-'))[1]);
...
//	  var dataForpc = $(pc).find('div.photo-ctn>a');//找出分组中的文字和图片
	  var time = $(pc).find('div.time').text();//图片文字的具体时间
          var words = $(pc).find('p.text').text();
          var picture = $(pc).find('p.hidden').attr('data-photo');;
...
//	  pictureAndContent.push(dataForpc.attr('href'));//装入图片
	  pictureAndContent.push(words);//装入文字
	  pictureAndContent.push(picture);//装入图片
	...
	theseDays.push(oneDay);//最后装入游记数组中
      ...
      dataArray.push(whichday_for_head);
